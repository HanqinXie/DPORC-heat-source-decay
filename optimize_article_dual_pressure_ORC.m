...
fluid_label = 'Isopentane';
WF = map_refprop_fluid(fluid_label);
HS_fluid = 'water';
cool_fluid = 'water';

source_temp_C = 150;
year_now = 0;
constrained_heat_source_outlet = false;
min_source_outlet_C = 70;

reference.enabled = true;
reference.P_e_lp_kPa = 1250;
reference.P_e_hp_kPa = 2530;
reference.T_sat_lp_K = 349.75;
reference.T_sat_hp_K = 386.45;
reference.W_net_kW = 3871;
reference.eta_thermal_percent = 10.22;
reference.eta_sys_percent = 7.066;
reference.m_HP_kg_s = 62.9;
reference.m_LP_kg_s = 32.8;
...
search.coarse.lp_n = 41;
search.coarse.hp_n = 41;
search.coarse.sh_n = 5;
search.coarse.initial_capacity = 512;

search.fine.name = 'refined';
search.fine.lp_n_full = 201;
search.fine.hp_n_full = 201;
search.fine.sh_n_full = 11;
...
base_const = struct();
base_const.WF = WF;
base_const.fluid_label = fluid_label;
base_const.HS_fluid = HS_fluid;
base_const.cool_fluid = cool_fluid;
base_const.source_temp_C = source_temp_C;
base_const.constrained_heat_source_outlet = constrained_heat_source_outlet;
base_const.min_source_outlet_K = min_source_outlet_C + 273.15;
...
filename = fullfile(script_dir, sprintf('%s_%gC_article_optimized_%s.csv', ...
    fluid_label, source_temp_C, timestamp));

fprintf('Optimized result will be written to %s\n', filename);
run_timer = tic;
[year_rows, year_log] = run_year_optimization(year_now, search, base_const);

if isempty(year_rows)
    error('optimize_article_dual_pressure_ORC:NoFeasibleSolution', ...
...
fprintf(['Article-style optimization at %.2f C finished with %d feasible rows. ' ...
         'Optimized P_lp = %.2f kPa, P_hp = %.2f kPa. ' ...
         'Best W_net = %.3f kW, eta_thermal = %.3f%%, eta_sys = %.3f%%. Elapsed %.2f s.\n'], ...
...
if reference.enabled
    fprintf(['Reference deltas: dP_lp = %.3f kPa, dP_hp = %.3f kPa, ' ...
             'dW = %.3f kW, deta_thermal = %.3f%%, deta_sys = %.3f%%, ' ...
             'dm_HP = %.3f kg/s, dm_LP = %.3f kg/s.\n'], ...
        best_row(20) - reference.P_e_lp_kPa, ...
        best_row(21) - reference.P_e_hp_kPa, ...
        best_row(3) - reference.W_net_kW, ...
        best_row(26) - reference.eta_thermal_percent, ...
        best_row(27) - reference.eta_sys_percent, ...
        best_row(28) - reference.m_HP_kg_s, ...
        best_row(29) - reference.m_LP_kg_s);
end
fprintf('Optimized results have been saved to %s\n', filename);
...
const.P_e_lp_min = base_const.P_e_lp_min;
const.P_e_lp_max = P_e_lp_max;
const.P_e_hp_min = base_const.P_e_hp_min;
const.P_e_hp_max = P_e_hp_max;
const.T8max_global = T_HS_in - base_const.delta_T_HAP_pp;
const.Ein = Ein;
const.eta_exergy_factor = eta_exergy_factor;

fprintf('Pressure bounds: P_lp = [%.2f, %.2f] kPa, P_hp = [%.2f, %.2f] kPa.\n', ...
    const.P_e_lp_min, const.P_e_lp_max, const.P_e_hp_min, const.P_e_hp_max);
fprintf('Stage 1 coarse search: %d x %d pressure points, %d x %d superheat points.\n', ...
...
year_rows = sort_candidate_rows(year_candidates);
end
    ...
    for i_hp = 1:numel(P_hp_list)
        ...
        for j_hp = 1:numel(T8_list)
            ...
            for j_lp = 1:numel(T4_list)
                ...
                if isfield(const, 'constrained_heat_source_outlet') && ...
                        const.constrained_heat_source_outlet && T_HS_out < const.min_source_outlet_K
                    continue;
                end
...
function sorted_rows = sort_candidate_rows(candidate_rows)
...
switch upper(string(fluid_label))
    case {"R600A", "I-C4", "IC4", "ISOBUTANE"}
        refprop_name = 'ISOBUTANE';
    case {"R600", "N-BUTANE", "BUTANE"}
        refprop_name = 'BUTANE';
    case "R1234YF"
    ...
    case "R245FA"
        refprop_name = 'R245FA';
    case {"R601", "N-PENTANE", "PENTANE"}
        refprop_name = 'PENTANE';
    case {"R601A", "I-C5", "IC5", "ISOPENTANE"}
        refprop_name = 'ISOPENTANE';
    otherwise
        refprop_name = char(fluid_label);
end
end
