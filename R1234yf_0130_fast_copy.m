...
WF = 'Pentane';   %R1234YF好了,R227EA好了,R1234ZEE好了,R600A好了,R236EA好了,R600好了,R245FA好了,R601A好了,R601好了
HS_fluid = 'water';
cool_fluid = 'water';

base_source_temp_C = 150;
year_step = 1;
time_years = 0:year_step:40;
n_years = numel(time_years);
...
T_cool_in = 20 + 273.15;
delta_T_cool = 5;
T_cool_out = T_cool_in + delta_T_cool;
delta_T_subcool = 2;
P_cool = 0.101 * 1e3;
eta_p = 0.75;
eta_T = 0.80;

T_cond = T_cool_out + delta_T_cond_pp;
...
base_const.base_source_temp_C = base_source_temp_C;
...
variable_names = { ...
    ...
    'ratio_Ede_pre_percent', 'ratio_Ede_hp_percent', 'ratio_Ede_lp_percent', ...
...
filename = fullfile(script_dir, sprintf('%s_40year_twostage_all_feasible_%s.csv', WF, timestamp));
rows_written = false;

fprintf('All feasible rows will be written to %s\n', filename);

for iy = 1:n_years
    year_now = time_years(iy);
    fprintf('\n=== Year %.1f (%d/%d) ===\n', year_now, iy, n_years);
    year_timer = tic;
    ...
    if isempty(year_rows)
        fprintf('Year %.1f: no feasible solution. Elapsed %.2f s.\n', year_now, toc(year_timer));
        continue;
    end

    year_table = array2table(year_rows, 'VariableNames', variable_names);
    if rows_written
        writetable(year_table, filename, 'WriteMode', 'append', 'WriteVariableNames', false);
    else
        writetable(year_table, filename);
        rows_written = true;
    end

    best_row = year_rows(1, :);
    fprintf(['Year %.1f wrote %d feasible rows. Best net_output = %.3f kW, eta_ex = %.3f%%, ' ...
             'P_lp = %.2f kPa, P_hp = %.2f kPa. Elapsed %.2f s.\n'], ...
        year_now, size(year_rows, 1), best_row(3), best_row(4), best_row(20), best_row(21), toc(year_timer));
end

if rows_written
    disp(['All feasible 20-year results have been saved to ', filename]);
else
    disp('No feasible solution rows were found, so no CSV was written.');
end

fprintf('Script finished in %.2f s.\n', toc(overall_timer));

function [year_rows, year_log] = run_year_optimization(year_now, search, base_const)
year_rows = zeros(0, 21);
year_log = struct('T_HS_in', NaN);

T_HS_in_C = heat_source_temp(year_now, base_const.base_source_temp_C);
if T_HS_in_C < 220
    T_HS_in = T_HS_in_C + 273.15;
else
    T_HS_in = T_HS_in_C;
end
year_log.T_HS_in = T_HS_in;
fprintf('Year %.1f source temperature: %.2f C.\n', year_now, T_HS_in - 273.15);
...
if ~(isfinite(P_e_lp_max) && isfinite(P_e_hp_max))
    fprintf('Year %.1f: pressure bounds are not finite.\n', year_now);
    return;
end

if P_e_lp_max <= base_const.P_e_lp_min || P_e_hp_max <= base_const.P_e_hp_min
    fprintf('Year %.1f: pressure bounds invalid for current heat source.\n', year_now);
    return;
end
...
fprintf('Stage 1 coarse search: %d x %d pressures, %d x %d superheat points.\n', ...
...
fine_lp_step = (P_e_lp_max - base_const.P_e_lp_min) / max(search.fine.lp_n_full - 1, 1);
...
if isempty(fine_candidates)
    ...
    year_candidates = coarse_candidates(:, 1:21);
else
    year_candidates = fine_candidates(:, 1:21);
end

[~, order] = sort(year_candidates(:, 3), 'descend');
year_rows = year_candidates(order, :);
end
        ...
        for j_hp = 1:numel(T8_list)
            ...
            for j_lp = 1:numel(T4_list)
                ...
                if lc > size(local_rows, 1)
                    grown_rows = zeros(size(local_rows, 1) * 2, 25);
