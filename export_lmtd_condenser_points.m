function export_lmtd_condenser_points(year_step)
% Export condenser LMTD points for the 9-fluid ref-style comparison figures.

if nargin < 1 || isempty(year_step)
    year_step = 5;
end
...
output_csv = fullfile(script_dir, 'figures_9fluids_comparison', '08_lmtd_condenser_integer_year_points.csv');

source_tbl = readtable(input_csv, 'TextType', 'string');
source_tbl = select_year_step_rows(source_tbl, year_step);
target_fluids = ["R1234YF", "R1234ZEE", "R227EA", "R236EA", "R245FA", "R600A", "R600", "R601A", "R601"];
years = unique(source_tbl.year, 'sorted');

cache_keys = strings(0, 1);
cache_values = [];
wide_tbl = table(years, 'VariableNames', {'year'});

for idx = 1:numel(target_fluids)
    fluid = target_fluids(idx);
    fluid_vals = nan(numel(years), 1);
    fluid_rows = source_tbl(upper(string(source_tbl.fluid)) == fluid, :);
    if isempty(fluid_rows)
        error('export_lmtd_condenser_points:MissingFluid', ...
            'No rows found for fluid %s after applying year step %d.', fluid, year_step);
    end

    params = build_default_params(map_refprop_fluid(fluid));
    for jdx = 1:height(fluid_rows)
        row = fluid_rows(jdx, :);
        year_value = double(row.year);
        key = build_cache_key(row);
        match_idx = find(cache_keys == key, 1);
        ...
        if isempty(match_idx)
            ...
            if ~details.feasible
                error('export_lmtd_condenser_points:%sInfeasible', fluid, ...
                    'Recovered point for %s at year %.3f is infeasible: %s', ...
                    fluid, year_value, details.fail_reason);
            end
            cache_keys(end + 1, 1) = key; %#ok<AGROW>
            if isempty(cache_values)
                cache_values = details;
            else
                cache_values(end + 1) = details; %#ok<AGROW>
            end
        else
            details = cache_values(match_idx);
        end

        year_idx = find(abs(years - year_value) < 1e-9, 1);
        fluid_vals(year_idx) = details.DT_cond_lmtd_K;
    end

    wide_tbl.(char(fluid)) = fluid_vals;
    fprintf('%s exported for %d year points.\n', fluid, sum(isfinite(fluid_vals)));
end

writetable(wide_tbl, output_csv);
fprintf('Input CSV: %s\n', input_csv);
fprintf('Year step: %d\n', year_step);
...
function selected = select_year_step_rows(source_tbl, year_step)
year_values = source_tbl.year;
rounded_years = round(year_values);
mask_integer = abs(year_values - rounded_years) < 1e-9;
mask_step = mod(rounded_years, year_step) == 0;
selected = source_tbl(mask_integer & mask_step, :);
if isempty(selected)
    error('export_lmtd_condenser_points:NoRows', ...
        'No rows matched the requested year step %d.', year_step);
end
end


function key = build_cache_key(row)
key = sprintf('%s|%.9f|%.9f|%.9f|%.9f|%.9f|%.9f|%.9f', ...
    char(string(row.fluid)), ...
    double(row.T_HS_in_K), ...
    double(row.P_e_lp_kPa), ...
    double(row.P_e_hp_kPa), ...
    double(row.net_output_kW), ...
    double(row.eta_ex_percent), ...
    double(row.lmtd_hp_K), ...
    double(row.lmtd_lp_K));
end
...
try
    ...
    DT_cond_inlet = T11 - T_cool_out;
    DT_cond_outlet = T1 - T_cool_in;
    DT_cond_lmtd = calc_lmtd(DT_cond_inlet, DT_cond_outlet);
    ...
    details.DT_cond_lmtd_K = DT_cond_lmtd;
...
fig = figure('Color', 'w', 'Position', [120, 120, 860, 650]);
ax = axes(fig);
set(ax, 'Position', [0.10, 0.12, 0.86, 0.82]);
hold(ax, 'on');

h_hot = plot(ax, q_hot_MW, t_hot_C, '-', ...
    'Color', [0.20, 0.20, 0.20], ...
    'LineWidth', 2.2, ...
    'DisplayName', '热源流体');

h_cold = plot(ax, q_cold_MW, t_cold_C, '-', ...
    'Color', [0.00, 0.45, 0.74], ...
    'LineWidth', 2.2, ...
    'DisplayName', '工质');

set(h_hot, 'DisplayName', cn_legend_hot());
set(h_cold, 'DisplayName', cn_legend_cold());
style_tq_axes(ax, fluid, year_value);
...
fig = figure('Color', 'w', 'Position', [80, 80, 1380, 680]);
tiledlayout(fig, 1, numel(results), 'Padding', 'loose', 'TileSpacing', 'compact');
...
for idx = 1:numel(results)
    ...
    set(h_hot, 'DisplayName', cn_legend_hot());
    set(h_cold, 'DisplayName', cn_legend_cold());
    style_tq_axes(ax, format_display_fluid(results(idx).fluid), year_value);
    if idx == 1
        lgd = legend(ax, 'Location', 'northwest', 'Box', 'off');
        set(lgd, 'FontName', 'Microsoft YaHei UI', 'FontSize', 11);
    end
...
title(ax, cn_title(fluid, year_value), ...
    'FontName', 'Microsoft YaHei UI', 'FontSize', 15, 'FontWeight', 'bold', 'Interpreter', 'none');
xlabel(ax, cn_label_x(), 'FontName', 'Microsoft YaHei UI', 'FontSize', 13, 'Interpreter', 'none');
ylabel(ax, cn_label_y(), 'FontName', 'Microsoft YaHei UI', 'FontSize', 13, 'Interpreter', 'none');
...
set(get(ax, 'Title'), 'FontName', 'Microsoft YaHei UI', 'FontWeight', 'bold', 'Interpreter', 'none');
set(get(ax, 'XLabel'), 'FontName', 'Microsoft YaHei UI', 'Interpreter', 'none');
set(get(ax, 'YLabel'), 'FontName', 'Microsoft YaHei UI', 'Interpreter', 'none');
grid(ax, 'off');
end
...
ylim(ax, [0.0, 150.0]);
set(ax, 'FontName', 'Times New Roman', 'FontSize', 11, 'LineWidth', 1.2, 'Box', 'on');
set(get(ax, 'YLabel'), 'String', ['Temperature T (' char(176) 'C)']);
set(get(ax, 'Title'), 'FontName', 'Times New Roman', 'FontWeight', 'bold', 'Interpreter', 'none');
set(get(ax, 'XLabel'), 'FontName', 'Times New Roman', 'Interpreter', 'none');
set(get(ax, 'YLabel'), 'FontName', 'Times New Roman', 'Interpreter', 'none');
grid(ax, 'off');
end
...
out = [char(string(fluid)), char(65292), sprintf('%.0f', year_value), ' ', char(24180)];
end


function out = cn_legend_hot()
out = char([28909, 28304, 27969, 20307]);
end


function out = cn_legend_cold()
out = char([24037, 36136]);
end
