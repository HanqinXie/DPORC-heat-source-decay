...
% 外层循环：低压蒸发压力 (并行)
step_lp = (P_e_lp_max - P_e_lp_min) / (num_steps_pressure - 1);
...
parfor i_lp = 1:num_steps_pressure
    % --- performance: collect solutions in a cell (avoid repeated reallocation) ---
    local_cells = cell(1, 4096);
    lc = 0;
    P_e_lp_local = P_e_lp_min + (i_lp - 1) * step_lp;
    % 计算低压相关状态
    ...
    % 状态点2 (泵1出口)
    P2 = P_e_lp_local;
    h2_ideal = refpropm_cached('H', 'P', P2, 'S', s1, WF);
    deltah_21 = (h2_ideal - h1) / eta_p;
    h2 = h1 + deltah_21;
    [s2, T2] = refpropm_cached('ST', 'P', P2, 'H', h2, WF);
    % 状态点3 (预热器1出口)
    P3 = P2;
    T3 = T_e_lp_local;
    ...
    % 状态点4'（低压蒸发器内，饱和蒸汽）
    P4p = P_e_lp_local;
    [T4p, h4p, s4p] = refpropm_cached('THS', 'P', P_e_lp_local, 'Q', 1, WF);
    % 内层循环：高压蒸发压力
    step_hp = (P_e_hp_max - max(P_e_hp_min, P_e_lp_local + 100)) / (num_steps_pressure - 1);
    for i_hp = 1:num_steps_pressure
        P_e_hp_local = max(P_e_hp_min, P_e_lp_local + 100) + (i_hp - 1) * step_hp;
        if P_e_hp_local > P_e_hp_max
            continue;
        end
        % 计算高压相关状态
        ...
        % 状态点5 (泵2出口)
        P5 = P_e_hp_local;
        h5_ideal = refpropm_cached('H', 'P', P5, 'S', s3, WF);
        deltah_53 = (h5_ideal - h3) / eta_p;
        h5 = h3 + deltah_53;
        [s5, T5] = refpropm_cached('ST', 'P', P5, 'H', h5, WF);
        % 状态点6 (预热器2出口)
        P6 = P_e_hp_local;
        T6 = T_e_hp_local;
        ...
        T8max = T_HS_in - delta_T_HAP_pp; % 修正为满足夹点
        T8min = T7 + 2; % 最小过热温度2 K
        k2_hp = (T8max - T8min) / (num_steps_superheat - 1);
        for j_hp = 1:num_steps_superheat
            % 状态点8 (过热器出口)
            P8 = P_e_hp_local;
            T8 = T8min + k2_hp * (j_hp - 1);
            ...
            k2_lp = (T4max - T4min) / (num_steps_superheat - 1);
            for j_lp = 1:num_steps_superheat
                % 状态点4 (低压蒸发器出口，过热蒸汽)
                P4 = P_e_lp_local;
                T4 = T4min + k2_lp * (j_lp - 1);
                ...
                % 状态点9 (高压涡轮出口)
                P9 = P_e_lp_local;
                h9s = refpropm_cached('H', 'P', P9, 'S', s8, WF);
                deltah_89 = (h8 - h9s) * eta_T;
                h9 = h8 - deltah_89;
                s9 = refpropm_cached('S', 'P', P9, 'H', h9, WF);
                % 状态点10 (混合点)
                P10 = P_e_lp_local;
                h10 = (m_O_LP * h4 + m_O_HP * h9) / m_f;
                [T10, s10] = refpropm_cached('TS', 'P', P10, 'H', h10, WF);
                % 状态点11 (低压涡轮出口)
                P11 = P_cond;
                h11s = refpropm_cached('H', 'P', P11, 'S', s10, WF);
                deltah_1011 = (h10 - h11s) * eta_T;
                h11 = h10 - deltah_1011;
                s11 = refpropm_cached('S', 'P', P11, 'H', h11, WF);
                T11 = refpropm_cached('T', 'P', P11, 'H', h11, WF);
                % 冷凝器状态点
                ...
                Qav = m_HS * (h_HS_in - h_HS_0); % 热源可提供的总热量
                % 热效率
                ...
                if abs(exerg_balance) < 0.1 && output > 0
                    ...
                    eta_ex_percent = output * 1000 / max(Ein, eps) * 100;

                    lc = lc + 1;
                    if lc > numel(local_cells)
                        local_cells{end+4096} = []; %#ok<AGROW>
                    end
                    local_cells{lc} = [ ...
    ...
    if lc == 0
        ...
        best_sols{i_lp} = vertcat(local_cells{1:lc});
    end
end
